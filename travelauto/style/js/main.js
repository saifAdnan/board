...
(function (factory) {
    ...
    //##############EMR FUNCITONS BLOCK START##################

    //Eumerations
    //1=BillingDone
    //2=BillingNotDone
    //3=Superbill created and not sent
    //4=Superbill Created and sent
    //5=Superbill F
    //6=Superbill E
    //7=Superbill Not created
    //8=Missed Appointment ICON
    //9=Notes Created for this Appointment
    //10=Notes Not Created for this Appointment
    //11=Reminder sent for this appointment
    //12=Reminder not sent for this appointment
    //13=Appointment already checkin
    //14=Checkin appointment
    //15=Appointment already checkedout
    //16=checkout appointment
    //17=Missed Appointment
    //18=Cancel Appointment
    //19=Reschedule Appointment
    //20=Confirmed Appointment
    //21=Edit Appointment
    //22=Double Book - Existing Pt.
    //23=Double Book -  New Pt.
    //24=Edit Demographics
    //25=Edit Insurance Policy
    //26=Add Comments
    //27=Emergency Appointment - New Pt.
    //28=Emergency Appointment - Established Pt.
    //29=Remove Recurring Blocked Appointment Time
    //30=Remove Blocked Appointment Time
    //31=Add Existing Patient Appointment
    //32=Add New Patient Appointment
    //33=Open Chart
    //34=Undo Checkin
    //35=Undo Checkout
    //}

    function GetDrillDownMenu(currentIndex, currentEvent) {
        var menuText = "<div id=\"divApptMenu" + currentIndex + "\" class=\"options pull-right\"><div class=\"drilldown-switcher\" style=\"font-size:12px;padding-left:5px;maring-top: -1px;color: black;\">" +
            "<div class=\"toggler\"><i class=\"fa fa-bars\"></i></div>" +
            "</div></div>";
        return menuText
    }
    //################### DRILL DOWN MENU OPTIONS FOR EACH APPOINTMENT BLOCK END###############################


    //this method is used to get the icons to show for each appointment based on conditions
    function GetAppointmentIcons(currentEvent,currentEventIndex) {

        //if appointmentid is not available for the current event then return empty string
        if (currentEvent.AppointmentID == undefined || currentEvent.AppointmentID == null || currentEvent.AppointmentID.toString().length <= 0 || currentEvent.AppointmentID <= 0) return "";

        var htmlIcons = "";
        htmlIcons +=
            "<div id=\"divApptIcons" + currentEventIndex + "\" class=\"options pull-right appointmenticons\" style=\"margin-top:-1px;color: black;background-color:white;\">";

        //###########billing done icons block start##################
        if (hasValue(currentEvent.SuperBillStatus) && currentEvent.SuperBillStatus == "1")
            htmlIcons += GetIcon('../UITheme/icons/EMR/Dollor_blue.png', 'Billing Done', currentEvent,'1');
        else
            htmlIcons += GetIcon('../UITheme/icons/EMR/Dollor.png', 'Billing not Done', currentEvent, '2');
        //###########billing done icons block end##################

        //###########super bill status icons block start##################
        if (hasValue(currentEvent.SuperBillStatus) && currentEvent.SuperBillStatus == "1")
            htmlIcons += GetIcon('../UITheme/icons/EMR/imgS_blue.png', 'Superbill Created and Not Sent', currentEvent, '3');
        else if (hasValue(currentEvent.SuperBillStatus) && currentEvent.SuperBillStatus == "2")
            htmlIcons += GetIcon('../UITheme/icons/EMR/imgP_blue.png', 'Superbill Created and Sent', currentEvent, '4');
        else if (hasValue(currentEvent.SuperBillStatus) && currentEvent.SuperBillStatus == "3")
            htmlIcons += GetIcon('../UITheme/icons/EMR/imgF_blue.png', 'F', currentEvent, '5');
        else if (hasValue(currentEvent.SuperBillStatus) && currentEvent.SuperBillStatus == "4")
            htmlIcons += GetIcon('../UITheme/icons/EMR/imgE_blue.png', 'E', currentEvent, '6');
        else
            htmlIcons += GetIcon('../UITheme/icons/EMR/imgS.png', 'Superbill not Created', currentEvent, '7');
        //###########super bill status icons block end##################

        //###########notes created icons block start##################
        if (hasValue(currentEvent.Notes) && currentEvent.Notes == "0")// --Missed
            htmlIcons += GetIcon('../UITheme/icons/EMR/imgM_blue.png', 'Missed Appointment', currentEvent, '8');
        else if (hasValue(currentEvent.Notes) && currentEvent.Notes == "1")//--NotesCreated
            htmlIcons += GetIcon('../UITheme/icons/EMR/Notes_blue.png', 'Notes Created for this Appointment', currentEvent, '9');
        else
            htmlIcons += GetIcon('../UITheme/icons/EMR/Notes_16.png', 'Notes not Created for this Appointment', currentEvent, '10');
        //###########notes created icons block end##################

        //###########appointment reminders icons block start##################
        if (hasValue(currentEvent.IsApptReminderSent) && currentEvent.IsApptReminderSent == true)// --Reminder Sentt
            htmlIcons += GetIcon('../UITheme/icons/EMR/Reminder_blue.png', 'Reminder sent for this Appointment', currentEvent, '11');
        else
            htmlIcons += GetIcon('../UITheme/icons/EMR/Reminder.png', 'Reminder not sent for this Appointment', currentEvent, '12');
        //###########appointment reminders icons block end##################

        //###########checkin appointment icons block start##################
        if (hasValue(currentEvent.CheckInTime) && currentEvent.CheckInTime.length > 0)
            htmlIcons += GetIcon('../UITheme/icons/EMR/checkin_blue.png', 'Appointment Checked In on ' + currentEvent.CheckInTime, currentEvent, '13');
        else
            htmlIcons += GetIcon('../UITheme/icons/EMR/checkin.png', 'Check In Appointment', currentEvent, '14');
        //###########checkin appointment icons block end##################

        //###########checkout appointment icons block start##################
        if (hasValue(currentEvent.CheckOutTime) && currentEvent.CheckOutTime.length > 0)
            htmlIcons += GetIcon('../UITheme/icons/EMR/checkin_blue.png', 'Appointment Checked Out on ' + currentEvent.CheckOutTime, currentEvent, '15');
        else {

            //if aready checkin then show checkout
            if (hasValue(currentEvent.CheckInTime) && currentEvent.CheckInTime.length > 0)
                htmlIcons += GetIcon('../UITheme/icons/EMR/checkin.png', 'Check Out this Appointment', currentEvent, '16');

        }

        //###########checkout appointment icons block end##################
    ...
    //get icon based on image source
    function GetIcon(imgsrc, tooltipmessage, currentEvent,apptEventCategory) {
        var imgHTML = "<img src=\"" + imgsrc + "\" tooltipmessage=\"" + tooltipmessage + "\" eventCategory=\"" + apptEventCategory + "\" style=\"padding-left:5px;\">";
        return imgHTML;
    }



    //################### BUILD CONTEXT MENUS FOR EMPTY SLOTS BLOCK START###############################
    //*******PURPOSE: this method is used to build context menu items for empty slots
    //*******CREATED BY:  George D
    //*******CREATED DATE: 07/24/2014
    //*******MODIFIED DEVELOPER: DATE - NAME - WHAT IS MODIFIED; xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    function buildEmptySlotContextMenu(targetElement, appointmentDate) {
        context.attach(targetElement, [
            { header: 'APPOINTMENTS'
            },
            {
                text: 'Add Existing Patient Appointment',
                href: '#',
                target: '_blank',
                cat: 'single',
                action: function (e) {
                    e.preventDefault();
                    $("#calendar").callbackEvents('eventCustomCallBack', this, appointmentDate, '31');
                }
            },
            {
                text: 'Add New Patient Appointment',
                href: '#',
                target: '_blank',
                cat: 'single',
                action: function (e) {
                    e.preventDefault();
                    $("#calendar").callbackEvents('eventCustomCallBack', this, appointmentDate, '32');
                }
            }
        ]);
    }
    ...
    //MODIFIED BY GEORGE TO DISPLAY AM AND PM IN MONTH VIEW
    ...
    $.fn.callbackEvents = function (options, currentElement, parmValue, eventCategory) {
        var args = Array.prototype.slice.call(arguments, 1); // for a possible method call
        var res = this; // what this function will return (this jQuery object by default)

        if (options == 'eventCustomCallBack') {
            this.each(function (i, _element) { // loop each DOM element involved
                var element = $(_element);
                var calendar = element.data('fullCalendar'); // get the existing calendar object (if any)

                calendar.trigger('eventCustomCallBack', this, currentElement, parmValue, eventCategory);
    ...
    function Header(calendar, options) {
        ...
        function renderSection(position) {
            var e = $("<td style='display:none;' class='fc-header-" + position + "'/>"); //modified by george to hide the header
    ...
    function AgendaView(element, calendar, viewName) {
        ...
        function slotBind(cells) {
            var date;
            //##############EMR MODIFICATION BLOCK START##################
            //building empty slot context menu
            if (cells != undefined && cells.length > 0) {
                for (var slotIndex = 0; slotIndex < cells.length; slotIndex++) {
                    date = cellToDate(cells[slotIndex]);
                    date.add(minTime + slotIndex * slotDuration);
                    date = calendar.rezoneDate(date);
                    buildEmptySlotContextMenu(cells[slotIndex], date);
                }
            }
            //##############EMR MODIFICATION BLOCK END##################
    ...
    function AgendaEventRenderer() {
        ...
        function renderSlotSegs(segs, modifiedEventId) {
            ...
            for (i = 0; i < segCnt; i++) {
                ...
                //##############EMR CODE BLOCK START##################
                // shave off space on right near scrollbars (2.5%)
                // TODO: move this to CSS somehow
                // columnRight -= columnWidth * .025; //Commenting this because we don not additional space right side corner (near scroll bars)
                //##############EMR CODE BLOCK END##################
                columnWidth = columnRight -columnLeft;
                ...
                html += slotSegHtml(event, seg, i);
            }

            slotSegmentContainer[0].innerHTML = html; // faster than html()
            eventElements = slotSegmentContainer.children();

            // modified by george to initiate all the drill down menus on that day
            //for (i = 0; i < segCnt; i++) {
            //   smallDivLeftDrilDownInit('divApptMenu' + i);
            //}

            //buildMenuForAppointmentSlot(segs);

            //buildEventsForAppointmentIcons(segs);

            //showing tooltip messages for appointment icons
            $("[tooltipmessage]").popover({
                'placement': 'bottom',
                'trigger': 'hover',
                'html': true,
                'container': 'body',
                'content': function () {
                    return $(this).attr("tooltipmessage");
                }
            });
        ...
        //GEORGE
        //this method is edited by george to do the changes in appointment div
        function slotSegHtml(event, seg, segIndex) {
            var html = "<";
            var url = event.url;
            ...
            var showCheckIn = 'block';
            if (event.isCheckedIn)
                showCheckIn = 'block'
            else
                showCheckIn = 'none';

            //var backColor = "106, 164, 193";
            //var foreColor = "255, 255, 255";
            var backColor = "255, 255, 255";
            var foreColor = "0, 0, 0";
            if (event.ApptBackColor != undefined && event.ApptBackColor.length > 0)
                backColor = event.ApptBackColor.substring(event.ApptBackColor.indexOf(",") +1);

            if (event.ApptForeColor != undefined && event.ApptForeColor.length > 0)
                foreColor = event.ApptForeColor.substring(event.ApptForeColor.indexOf(",") +1);
            ...
            html +=
                ...
                "'" +
                ">" +
                "<div class='fc-event-inner' style='background-color: rgb(" + backColor + "); color: rgb(" + foreColor + ");'>" +       //#6AA4C1  RGB(106, 164, 193)
                "<div class=\"options pull-left\" style=\"padding-left: 5px;\">" +
                event.AppointmentInfo +
                "</div>" +
                GetDrillDownMenu(segIndex, event) +            //adding drill down menu

                //adding icons block start
                GetAppointmentIcons(event, segIndex) +
                //adding icons block end
    ...
    function DayEventRenderer() {
        ...
        function buildHTMLForSegment(segment) {
            ...
            html +=
                ...
                "'" +
                ">" +
                "<div class='fc-event-inner' style='background-color: rgb(" + backColor + "); color: rgb(" + foreColor + ");'>";
            ...
            html +=
                ...
                htmlEscape(event.PersonLastNameFirstName || '') + //modified by george to show patient info in month view
                "</span>" +
                "</div>";
...
//##############EMR FUNCITONS BLOCK START##################
//raising custom call back event
function eventCustomCallBack(currentElement, parmValue, eventCategory) {
    event.stopPropagation();
    $("#calendar").callbackEvents('eventCustomCallBack', currentElement, parmValue, eventCategory);
};
//##############EMR FUNCITONS BLOCK END##################
